...
> 두 페이지 펼침 / CBR / Onyx 특화 모드 / 세로 스크롤(웹툰)은 v1.5 이후로 미뤘다.
> 세로 스크롤은 e-ink 잔상/갱신 속도 한계로 표준 안드로이드 API 만으론 만족스러운
> UX가 어렵다고 판단(2026-05-08). 자세한 이유는 §4 비목표 참조.

#### 읽기 코어 (macOS Panely에서 그대로 포팅)
- [ ] **단일 페이지 모드만 v1.0** — 세로 스크롤(웹툰)은 v1.5
- [ ] LTR / RTL 토글
...
#### v1.0에서 미룬 것
- [ ] **세로 스크롤(웹툰) 모드** — vertical paginated(viewport 단위 분할 + 흰 줄 컷
  보호) 또는 Onyx fast-mode 연동 중 선택. 표준 안드로이드 fling 스크롤은 잔상으로 부적합
