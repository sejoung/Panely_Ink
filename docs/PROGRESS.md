...
- **2026-05-09** — M4-A 시리즈 컨텍스트 인프라. 책 1권만 받던 reader가 형제 권 목록도 함께 받도록 변경 — 다음/이전 권 카드(M4-1·2), 시리즈 설정 propagate(M4-5)의 공통 토대. (1) `reader/SeriesContext.kt` 신규 — `current: BookEntry` + `siblings: List<BookEntry>` data class. `currentIndex`/`previousBook`/`nextBook`/`isFirst`/`isLast` 헬퍼. nested 책 구분은 `bookIdSource`로(stableKey와 동일 키). (2) `LibraryList`/`LibraryGrid` `onOpenBook` 시그니처 `(BookEntry, List<BookEntry>) -> Unit`로 확장 — 그리드/리스트 각각 entries에서 BookEntry만 자연순으로 미리 추려 클로저에 캡처. ZIP-of-CBZ 가상 폴더에 들어와 nested 책 클릭 시 자동으로 `nestedBooks`가 형제. (3) `LibraryScreen.onOpenBook: (SeriesContext) -> Unit` — `viewModel.openBook` 결과 콜백에서 SeriesContext로 wrap. (4) `MainActivity.Screen.Reader(val context: SeriesContext)` + `onNavigate(newCurrent)` — siblings 그대로 두고 current만 `copy()`로 교체해 새 ReaderScreen 재진입. (5) `ReaderScreen` 시그니처 `entry: BookEntry` → `context: SeriesContext` + `onNavigate: (BookEntry) -> Unit = {}`. 내부 `entry`는 `context.current`로 derive. `produceState` 키도 `documentUri` → `bookIdSource`로 변경(nested 책 형제 권 간 이동에서 정확히 재진입). UI(다음 권 카드/메뉴 ◀▶)는 후속 B/C 단계 일반 폴더는 권수/표지가 보이는데 시리즈 zip은 책으로 표시되어 둘 다 누락된 일관성 문제. (1) **`LibraryRepository.seriesCache`** — `inspectZipForSeries` 결과(시리즈/non-시리즈 둘 다) in-memory 영속. 같은 폴더 재진입에서 ZIP 재오픈 0번. (2) **`LibraryViewModel.batchInspectSeries`** — `refreshChildren`/single-root 자동 진입 후 백그라운드로 .zip BookEntry들을 `inspectSemaphore(2)`로 차례차례 검사 → 시리즈인 zip은 entries 안에서 같은 자리에 FolderEntry로 점진적 swap. 화면 상단 zip부터 폴더로 변신. (3) **가상 폴더 권수** — `requestFolderCount`에 nested 분기, `nestedBooks.size` 즉시 set. countBooks/SAF query 0번. (4) **가상 폴더 표지 stream 디코드** — `CbzArchive.firstImageBytesInNested(entryName)` + `CoverExtractor.extractNestedCover(parentUri, entryName)`. 부모 ZIP을 한 번 열어 `ZipArchiveInputStream`으로 nested cbz를 메모리에서 streaming → 첫 image entry bytes만 `BitmapFactory.decodeByteArray`로 디코드. 디스크 추출(`NestedZipExtractor`) 없이도 표지 표시. (5) `requestCover` nested 분기를 fallback 체인으로: reader 추출 캐시 우선 → 없으면 stream 디코드 → 둘 다 실패면 FAILED 영구 저장. (6) `requestFolderCover`에 nested 분기 — `folder.nestedBooks?.firstOrNull()`로 SAF query 생략. **결과**: ZIP-of-CBZ가 라이브러리에서 일반 폴더와 동일한 권수+표지 표시
